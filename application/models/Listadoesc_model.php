...
class Listadoesc_model extends CI_Model
{
  function niveles($idmunicipio=0){
    ...
    function niveles_estadistica($idmunicipio=0){
    if ($idmunicipio>0) {
      $where = "WHERE ct.idmunicipio= {$idmunicipio}";
    }
    else {
      ...
      $query="SELECT
              ...
              INNER JOIN centrocfg cfg ON ct.idct = cfg.idct
              INNER JOIN niveleducativo n ON cfg.nivel = n.idnivel
              {$where} AND n.idnivel IN(1,2,3,4)
              GROUP BY n.idnivel
              ORDER BY FIELD(n.idnivel,1,2,3,4)";
      return  $this->db->query($query)->result_array();
    }// niveles()

    
    function niveles_riesgo($idmunicipio=0){
    if ($idmunicipio>0) {
      $where = "WHERE ct.idmunicipio= {$idmunicipio}";
    }
    else {
      ...
      $query="SELECT
              ...
              INNER JOIN centrocfg cfg ON ct.idct = cfg.idct
              INNER JOIN niveleducativo n ON cfg.nivel = n.idnivel
              {$where} AND n.idnivel IN(2,3)
              GROUP BY n.idnivel
              ORDER BY FIELD(n.idnivel,2,3)";
      return  $this->db->query($query)->result_array();
    }// niveles()
