...
class Reportes extends CI_Controller {
	...
	public function est_generales_xmuni(){
				...
				$obj_excel->getActiveSheet()->SetCellValue('A2', 'Municipio: '.$municipio.', Nivel: '.$nivel.', Sostenimiento: '.$sostenimiento.', Modalidad: '.$modalidad.', Ciclo escolar: '.$ciclo.'');
				...
				foreach ($result_alumnos as $row) {
					...
					$obj_excel->getActiveSheet()->SetCellValue('F'.$aux, $row['alumn_t_t'] );
					$obj_excel->getActiveSheet()->SetCellValue('G'.$aux, number_format($row['alumnos1']) );
					$obj_excel->getActiveSheet()->SetCellValue('H'.$aux, number_format($row['alumnos2']) );
					$obj_excel->getActiveSheet()->SetCellValue('I'.$aux, number_format($row['alumnos3']) );
					$obj_excel->getActiveSheet()->SetCellValue('J'.$aux, number_format($row['alumnos4']) );
					$obj_excel->getActiveSheet()->SetCellValue('K'.$aux, number_format($row['alumnos5']) );
					$obj_excel->getActiveSheet()->SetCellValue('L'.$aux, number_format($row['alumnos6']) );
				...
				foreach ($result_docentes as $row) {
					...
					$obj_excel->getActiveSheet()->SetCellValue('F'.$aux, $row['docentes_t_g'] );
				...
				$aux++;
				$obj_excel->getActiveSheet()->SetCellValue('A'.$aux, 'ciclo escolar ');
				$obj_excel->getActiveSheet()->mergeCells('A'.$aux.':C'.$aux);
				$obj_excel->getActiveSheet()->getStyle('A'.$aux.':C'.$aux)->applyFromArray($this->style_titulo);
				$aux++;
